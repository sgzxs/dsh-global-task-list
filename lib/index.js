...
const TASK_STATUSES = ['pending', 'running', 'done', 'blocked', 'failed']
...
async function getTask(tasks, id) {
  for await (const [key, value] of tasks.entries()) {
...
export default {
  name,
  inject: ['tools', 'storageDomain', 'webServer', 'jobs', 'agents'],
  apply(ctx, config) {
    ...
    const tasksPromise = domainPromise.then((domain) => domain.table('tasks'))
    ...
    let syncing = false
    const syncFromJobs = async (owner) => {
      if (syncing) return
      syncing = true
      try {
        ...
        for (const snap of snapshots) {
          ...
          for await (const [key, value] of tasks.entries()) {
            ...
            if (value.status !== mapped) {
              await tasks.put(key, { ...value, status: mapped, updatedAt: Date.now() })
            }
          }
        }
      } catch (error) {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if (req.method === 'POST' && url === '/task-ui/ask') {
            ...
            const masters = agents.filter((a) => a.session.header?.meta?.origin !== 'subagent')
    ...
    ctx.tools.register(defineTool({
      ...
      async execute(args) {
        ...
        await tasks.put(args.id, { ...task, ...patch, updatedAt: Date.now() })
